...
import { signInWithGoogle, signInWithTwitter, signInWithApple, signInWithEmail, signUpWithEmail } from '../data/supabase';
...
export const SocialLoginPanel: React.FC = () => {
  ...
  const handleSocial = async (provider: 'google' | 'twitter' | 'apple') => {
    setLoading(provider);
    setError('');
    try {
      const fn = provider === 'google' ? signInWithGoogle
        : provider === 'twitter' ? signInWithTwitter
        : signInWithApple;
  ...
  const socialButtons = [
    ...
    { id: 'apple' as const, label: 'Continuar con Apple', icon: <AppleIcon />, style: { background: 'rgba(0,0,0,0.35)', border: '1px solid rgba(255,255,255,0.08)', color: 'rgba(254,248,237,0.75)' } },
  ];
